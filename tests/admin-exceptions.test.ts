...
import {
  ...
  EXCEPTION_GROUPS,
} from '../src/features/admin/exceptions/model/admin-exception';
...
describe('ADMIN Exception list and export service', () => {
  it('seeds every canonical detail group plus the list screen', async () => {
    const result = await adminExceptionService.list(
      ADMIN_EXCEPTION_DEFAULT_QUERY,
      'CMS_ADMIN',
    );
    assert.equal(result.items.length, 6);
    assert.deepEqual(
      new Set(result.items.map(({ group }) => group)),
      new Set(EXCEPTION_GROUPS),
    );
  });
  ...
  it('paginates in newest-first order', async () => {
    const result = await adminExceptionService.list(
      { ...ADMIN_EXCEPTION_DEFAULT_QUERY, pageSize: 2 },
      'CMS_ADMIN',
    );
    ...
    assert.deepEqual(
      result.items.map(({ id }) => id),
      ['exception-persistence-001', 'exception-cancel-refund-001'],
    );
  });
  ...
  it('exports all filtered rows through a mock request lifecycle', async () => {
    const request = await adminExceptionService.requestExport(
      { ...ADMIN_EXCEPTION_DEFAULT_QUERY, pageSize: 1, status: 'OPEN' },
      'CMS_ADMIN',
      'cms-admin',
    );
    assert.equal(request.status, 'PROCESSING');
    assert.equal(request.rowCount, 5);
    const completed = await adminExceptionService.completeExport(
      request.id,
      'CMS_ADMIN',
    );
  ...
  it('enforces view and export permissions in the service', async () => {
    ...
    await assert.rejects(
      () =>
        adminExceptionService.requestExport(
          ADMIN_EXCEPTION_DEFAULT_QUERY,
          'CMS_FINANCE',
          'cms-finance',
        ),
...
describe('ADMIN Exception detail and retry service', () => {
  it('returns each discriminated detail layout with resolved references', async () => {
    ...
    const details = await Promise.all(
      ids.map((id) => adminExceptionService.getDetail(id, 'CMS_ADMIN')),
    );
    assert.deepEqual(
      details.map(({ exception }) => exception.group),
      EXCEPTION_GROUPS,
    );
    assert.equal(
      details[0]?.exception.group === 'REQUEST_AUTHENTICATION' &&
        details[0].exception.details.endpoint,
      'POST /orders/success',
    );
    assert.equal(
      details[1]?.exception.group === 'CLICK_ELIGIBILITY' &&
        details[1].exception.details.checks.length,
      3,
    );
    assert.equal(
      details[2]?.exception.group === 'BRAND_COMMISSION' &&
        details[2].exception.details.items[0]?.validationResult,
      'FAILED',
    );
    assert.equal(
      details[3]?.exception.group === 'TENANT_SHARE' &&
        details[3].exception.details.items[0]?.validationResult,
      'PASS',
    );
    assert.equal(
      details[4]?.exception.group === 'CANCEL_REFUND' &&
        details[4].relatedTransaction?.id,
      'transaction-lotus-pending',
    );
    assert.equal(
      details[5]?.exception.group === 'TRANSACTION_PERSISTENCE' &&
        details[5].exception.details.failureCode,
      'DB_COMMIT_TIMEOUT',
    );
  ...
  it('rejects unknown details and unauthorized roles', async () => {
    await assert.rejects(
      () => adminExceptionService.getDetail('missing', 'CMS_ADMIN'),
      /EXCEPTION_NOT_FOUND/,
    );
    await assert.rejects(
      () =>
        adminExceptionService.getDetail('exception-auth-001', 'CMS_OPERATION'),
      /FORBIDDEN/,
    );
  });
  ...
  it('simulates failed retry without changing transaction output', async () => {
    ...
    const result = await adminExceptionService.retry(
      'exception-cancel-refund-001',
      'CMS_ADMIN',
      'cms-admin',
      'FAILED',
