...
} from '../src/features/tenant/roles/model/tenant-role-permissions';
...
describe('TENANT role and permission foundation', () => {
  it('seeds four immutable Active system roles and custom roles per Tenant', () => {
    for (const tenantId of ['tenant-lotus', 'tenant-bamboo']) {
      const roles = mockData.tenantRoles.filter(
      ...
      for (const role of roles.filter(({ type }) => type === 'SYSTEM')) {
        ...
        assert.deepEqual(
          ...
          getPermissionsForRole(role.code as 'TENANT_ADMIN'),
        );
      }
    }
  });
  ...
  it('defines exactly seven modules and derives Full, indeterminate and coverage states', () => {
    ...
    assert.deepEqual(
      getModuleSelection(['brands.view', 'brands.edit'], ['brands.view']),
      { selectedCount: 1, full: false, indeterminate: true },
    );
    assert.deepEqual(
      getModuleSelection(
      ...
      { selectedCount: 2, full: true, indeterminate: false },
    );
    assert.equal(
      getPermissionCoverage([...getPermissionsForRole('TENANT_ADMIN')]),
      7,
    );
  });
  ...
  it('rejects Platform permissions and accepts only the Tenant catalog', () => {
    ...
    assert.equal(
      isValidTenantPermissionSet(['brands.view', 'configuration.delete']),
      false,
    );
  });

  it('counts role assignment and protects the last account with role/account management access', () => {
    const adminRole = mockData.tenantRoles.find(
      ({ tenantId, code }) =>
        tenantId === 'tenant-lotus' && code === 'TENANT_ADMIN',
    );
    assert.ok(adminRole);
