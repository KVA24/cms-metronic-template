...
describe('ADMIN Transaction list and export service', () => {
  it('lists deterministic financial projections from current item states', async () => {
    ...
    const pending = result.items.find(
      ({ id }) => id === 'transaction-lotus-pending',
    )!;
    ...
    const cancelled = result.items.find(
      ({ id }) => id === 'transaction-lotus-cancelled',
    )!;
  ...
  it('creates and completes an export request for all filtered rows, not the page', async () => {
    const request = await adminTransactionService.requestExport(
      { ...ADMIN_TRANSACTION_DEFAULT_QUERY, pageSize: 1 },
      'CMS_ADMIN',
      'cms-admin',
    );
    ...
    const completed = await adminTransactionService.completeExport(
      request.id,
      'CMS_ADMIN',
    );
  ...
  it('enforces export permission independently from list permission', async () => {
    await assert.rejects(
      () =>
        adminTransactionService.requestExport(
          ADMIN_TRANSACTION_DEFAULT_QUERY,
          'CMS_CSKH',
          'cms-cskh',
        ),
...
describe('ADMIN Transaction detail service', () => {
  it('returns header, item snapshots and immutable chronological history', async () => {
    const detail = await adminTransactionService.getDetail(
      'transaction-lotus-pending',
      'CMS_ADMIN',
    );
    ...
    assert.deepEqual(
      detail.items.map(({ status }) => status),
      ['CONFIRMED', 'PENDING'],
    );
    assert.deepEqual(
      detail.histories.map(({ eventType }) => eventType),
      ['ORDER_RECORDED', 'ITEM_CONFIRMED'],
    );
  ...
  it('returns confirmed date only for a fully Confirmed order', async () => {
    const confirmed = await adminTransactionService.getDetail(
      'transaction-bamboo-confirmed',
      'CMS_ADMIN',
    );
    const pending = await adminTransactionService.getDetail(
      'transaction-lotus-pending',
      'CMS_ADMIN',
    );
  ...
  it('applies the same financial role scope to item detail', async () => {
    const cskh = await adminTransactionService.getDetail(
      'transaction-lotus-pending',
      'CMS_CSKH',
    );
  ...
  it('rejects a missing Transaction', async () => {
    await assert.rejects(
      () => adminTransactionService.getDetail('missing', 'CMS_ADMIN'),
      /NOT_FOUND/,
    );
  });
});
