...
describe('TENANT password recovery service', () => {
  it('validates a registered TENANT email and creates a five-minute demo OTP request', async () => {
    await assert.rejects(() =>
      tenantPasswordRecoveryService.requestReset({ email: 'invalid' }),
    );
    await assert.rejects(
      () =>
        tenantPasswordRecoveryService.requestReset({
          email: 'missing@example.test',
        }),
      /EMAIL_NOT_FOUND/,
    );
    const result = await tenantPasswordRecoveryService.requestReset({
      email: ' viewer@lotus.test ',
    });
  ...
  it('keeps invalid and expired OTPs unverified and supports resend', async () => {
    const request = await tenantPasswordRecoveryService.requestReset({
      email: 'viewer@lotus.test',
    });
    await assert.rejects(
      () =>
        tenantPasswordRecoveryService.verifyOtp({
    ...
    const verified = await tenantPasswordRecoveryService.verifyOtp({
      requestId: request.id,
      otp: DEMO_RESET_OTP,
    });
  ...
  it('requires verified OTP and matching policy-compliant passwords', async () => {
    ...
    await assert.rejects(() =>
      tenantPasswordRecoveryService.resetPassword({
        requestId: request.id,
  ...
  it('updates the shared login password once and does not unlock a locked account', async () => {
    ...
    await tenantPasswordRecoveryService.verifyOtp({
      requestId: request.id,
      otp: DEMO_RESET_OTP,
    });
    await tenantPasswordRecoveryService.resetPassword({
      requestId: request.id,
      newPassword: 'NewTenant123!',
      confirmPassword: 'NewTenant123!',
    });
    await assert.rejects(
      () =>
        tenantPasswordRecoveryService.resetPassword({
          requestId: request.id,
          newPassword: 'AgainTenant123!',
          confirmPassword: 'AgainTenant123!',
        }),
    ...
    const lockedRequest = await tenantPasswordRecoveryService.requestReset({
      email: account.email,
    });
    await tenantPasswordRecoveryService.verifyOtp({
    ...
    await tenantPasswordRecoveryService.resetPassword({
      requestId: lockedRequest.id,
      newPassword: 'LockedNew123!',
      confirmPassword: 'LockedNew123!',
    });
