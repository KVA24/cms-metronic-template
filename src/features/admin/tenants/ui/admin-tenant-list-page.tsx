    ...
    <Container className="space-y-6 py-6 lg:py-8">
      ...
      <Card>
        <CardContent className="pt-6">
          <form className="grid gap-4 md:grid-cols-2 xl:grid-cols-4" onSubmit={apply}>
            <label className="space-y-1">
              <span className="text-sm font-medium">{t('ADMIN_TENANTS.KEYWORD')}</span>
              <Input
                id="tenant-keyword"
                name="keyword"
                maxLength={100}
            ...
            <label className="space-y-1">
              <span className="text-sm font-medium">{t('ADMIN_TENANTS.OWNER')}</span>
              <select
                id="tenant-owner"
                name="accountOwner"
                className={selectClassName}
                ...
                onChange={(event) =>
                  setFilters((current) => ({ ...current, accountOwner: event.target.value }))
                }
              >
            ...
            <label className="space-y-1">
              <span className="text-sm font-medium">{t('ADMIN_TENANTS.UPDATED_PERIOD')}</span>
              <select
                ...
                onChange={(event) =>
                  setFilters((current) => ({
                    ...current,
                    updatedPeriod: event.target.value as AdminTenantQuery['updatedPeriod'],
                  }))
                }
              >
                <option value="ALL">{t('COMMON.ALL')}</option>
                <option value="7_DAYS">{t('ADMIN_TENANTS.LAST_7_DAYS')}</option>
                <option value="30_DAYS">{t('ADMIN_TENANTS.LAST_30_DAYS')}</option>
              </select>
            </label>
            <div className="flex gap-2 md:col-span-2 xl:col-span-4">
              <Button type="submit" variant="mono"><Search />{t('COMMON.APPLY')}</Button>
              <Button type="button" variant="outline" onClick={() => setQuery(ADMIN_TENANT_DEFAULT_QUERY)}>
                <RotateCcw />{t('COMMON.RESET')}
              </Button>
              <Button type="button" variant="outline" onClick={() => result.refetch()}>
      ...
      <Card>
        <CardContent className="space-y-4 pt-6">
          <p className="text-sm text-muted-foreground">
            {t('ADMIN_TENANTS.RESULT_COUNT', { count: result.data?.totalItems ?? 0 })}
          </p>
          {result.isLoading ? (
                ...
                {result.data?.items.map((tenant) => (
                  <TableRow key={tenant.id}>
                    <TableCell>
                      ...
                      <p className="font-mono text-xs text-muted-foreground">{tenant.code} · {tenant.id}</p>
                    </TableCell>
                    ...
                    <TableCell>{tenant.brandCount} / {tenant.offerCount}</TableCell>
                    <TableCell>{tenant.revenueShareCount || '-'}</TableCell>
                    <TableCell>
                      <p>{dateFormatter.format(new Date(tenant.updatedAt))}</p>
                      <p className="text-xs text-muted-foreground">{tenant.updatedBy}</p>
                    </TableCell>
                    <TableCell><div className="flex justify-end gap-2">
                      <Button variant="outline" size="icon" asChild>
                        <Link to={`/admin/tenants/${tenant.id}`} aria-label={t('ADMIN_TENANTS.VIEW', { name: tenant.name })}><Eye /></Link>
                      </Button>
                      {tenant.canEdit && <Button variant="outline" size="icon" asChild>
                        <Link to={`/admin/tenants/${tenant.id}?edit=true`} aria-label={t('ADMIN_TENANTS.EDIT', { name: tenant.name })}><Pencil /></Link>
                      </Button>}
                    </div></TableCell>
                  </TableRow>
                ))}
          ...
          {result.data && result.data.totalItems > 0 && (
            <div className="flex items-center justify-end gap-2 border-t pt-4">
              <span className="text-sm text-muted-foreground">
                {t('ADMIN_TENANTS.PAGE', { page: result.data.page, total: result.data.totalPages })}
              </span>
              <Button variant="outline" size="icon" disabled={result.data.page <= 1} aria-label={t('COMMON.PREVIOUS')} onClick={() => setQuery({ ...query, page: query.page - 1 })}><ChevronLeft /></Button>
              <Button variant="outline" size="icon" disabled={result.data.page >= result.data.totalPages} aria-label={t('COMMON.NEXT')} onClick={() => setQuery({ ...query, page: query.page + 1 })}><ChevronRight /></Button>
            </div>
          )}
        </CardContent>
      </Card>
