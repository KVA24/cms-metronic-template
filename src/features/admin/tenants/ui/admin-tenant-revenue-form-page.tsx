...
export function AdminTenantRevenueFormPage() {
  ...
  const result = useAdminTenantRevenue(tenantId, brandId, session?.roleCode as AdminRoleCode);
  ...
  return <AdminTenantRevenueEditor key={result.data.config?.version ?? 'new'} data={result.data} />;
}
