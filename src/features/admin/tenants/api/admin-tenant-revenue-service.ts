...
function context(tenantId: string, brandId: string) {
  ...
  const categories = mockData.brandCategoryMappings.flatMap((mapping) => {
      ...
      return [{ id: category.id, name: category.contents.find(({ locale }) => locale === 'vi-VN')?.name ?? category.code }];
    });
  const offers = mockData.offers.flatMap(({ id, brandId: offerBrandId, title, status }) =>
...
export const adminTenantRevenueService = {
  async listRevenueShares(
    ...
    const items = mockData.tenantBrandAssignments.flatMap((item) => {
        ...
        if (keyword && ![brand.id, brand.name].some((value) => value.toLowerCase().includes(keyword))) return [];
        ...
        return [{
          ...
          categoryOverrideCount: config?.overrides.filter(({ type }) => type === 'CATEGORY').length ?? 0,
          offerOverrideCount: config?.overrides.filter(({ type }) => type === 'OFFER').length ?? 0,
          status: config?.status ?? null,
          updatedAt: config?.updatedAt ?? null,
        }];
      });
    return structuredClone({ tenant, items, canEdit: hasPermission(roleCode, 'tenants.revenue_share.edit') });
  },

  async getRevenueShare(tenantId: string, brandId: string, roleCode: AdminRoleCode) {
    assertPermission(roleCode, 'tenants.revenue_share.view');
    ...
    return structuredClone({
      ...current,
      visible: current.assigned.showOnLanding && current.brand.status === 'ACTIVE',
      config,
  ...
  async saveRevenueShare(
    ...
    if (parsed.status === 'ACTIVE' && (!currentContext.assigned.showOnLanding || currentContext.brand.status !== 'ACTIVE'))
    ...
    parsed.overrides.forEach((override) => {
      if (override.type === 'CATEGORY' && !categoryIds.has(override.targetId)) throw new Error('TARGET_INVALID');
      if (override.type === 'OFFER' && !offerIds.has(override.targetId)) throw new Error('TARGET_INVALID');
    });
    const sequence = mockData.tenantRevenueShares.length + 1;
    const next: TenantRevenueShare = {
      id: current?.id ?? `revenue-${tenantId.replace('tenant-', '')}-${brandId.replace('brand-', '')}`,
      tenantId,
      brandId,
      ...
      overrides: parsed.overrides.map((override, index) => ({
        id: override.clientId.startsWith('revenue-') ? override.clientId : `revenue-override-${sequence}-${index + 1}`,
        type: override.type,
    ...
    mockData.auditRecords.push({ id: `audit-revenue-${mockData.auditRecords.length + 1}`, actorId, action: 'SAVE_TENANT_REVENUE_SHARE', entityType: 'TENANT_REVENUE_SHARE', entityId: next.id, occurredAt: next.updatedAt });
...
export function resolveTenantShareRate(
  ...
  const offer = config.overrides.find((rule) => rule.type === 'OFFER' && rule.targetId === offerId && rule.status === 'ACTIVE');
  if (offer) return offer.rate;
  const category = config.overrides.find((rule) => rule.type === 'CATEGORY' && rule.targetId === categoryId && rule.status === 'ACTIVE');
