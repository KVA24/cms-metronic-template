...
const accountKeys = {
  all: (tenantId: string) => [...adminTenantKeys.detail(tenantId), 'accounts'] as const,
  list: (
    tenantId: string,
...
export function useAdminTenantAccounts(
  ...
  return useQuery({
    ...
    queryFn: () => adminTenantAccountService.listAccounts(tenantId, query, roleCode),
    retry: false,
  });
}
...
export function useCreateAdminTenantAccount(tenantId: string) {
  ...
  return useMutation({
    mutationFn: ({ input, roleCode, actorId }: {
      input: AdminTenantAccountCreateInput;
      roleCode: AdminRoleCode;
      actorId: string;
    }) => adminTenantAccountService.createAccount(tenantId, input, roleCode, actorId),
...
export function useUpdateAdminTenantAccount(tenantId: string) {
  const invalidate = useInvalidateAccounts();
  return useMutation({
    mutationFn: ({ accountId, input, expectedVersion, roleCode, actorId }: {
      accountId: string;
...
export function useDisableAdminTenantAccount(tenantId: string) {
  const invalidate = useInvalidateAccounts();
  return useMutation({
    mutationFn: ({ accountId, roleCode, actorId }: {
      accountId: string;
      roleCode: AdminRoleCode;
      actorId: string;
    }) => adminTenantAccountService.disableAccount(tenantId, accountId, roleCode, actorId),
    onSuccess: invalidate,
  });
}
