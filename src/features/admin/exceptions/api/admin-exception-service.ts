...
function filtered(query: AdminExceptionQuery) {
  if (Object.keys(validateExceptionDateRange(query)).length) throw new Error('DATE_RANGE_INVALID');
  const keyword = query.keyword.trim().toLowerCase();
  return mockData.exceptions.filter((item) => {
    if (keyword && ![item.id, item.orderId, item.brandOrderId].some((value) => value?.toLowerCase().includes(keyword))) return false;
    ...
    if (query.dateFrom && item.createdAt < `${query.dateFrom}T00:00:00.000Z`) return false;
    if (query.dateTo && item.createdAt > `${query.dateTo}T23:59:59.999Z`) return false;
    return true;
  });
}
...
function listItem(item: (typeof mockData.exceptions)[number]) {
  ...
  return { ...item, details: undefined, tenantName: tenant?.name ?? null, brandName: brand?.name ?? null };
}
...
export const adminExceptionService = {
  async list(query: AdminExceptionQuery, roleCode: AdminRoleCode) {
    ...
    const items = filtered(query).sort((left, right) => right.createdAt.localeCompare(left.createdAt));
    const totalItems = items.length;
    const totalPages = Math.max(1, Math.ceil(totalItems / query.pageSize));
    const page = Math.min(query.page, totalPages);
    const start = (page - 1) * query.pageSize;
    return structuredClone({
      items: items.slice(start, start + query.pageSize).map(listItem), page, pageSize: query.pageSize, totalItems, totalPages,
      canExport: hasPermission(roleCode, 'exceptions.export'), canRetry: hasPermission(roleCode, 'exceptions.retry'),
    });
  },
  ...
  async requestExport(query: AdminExceptionQuery, roleCode: AdminRoleCode, actorId: string) {
    ...
    const request = { id: `exception-export-${sequence}`, type: 'EXCEPTION' as const, status: 'PROCESSING' as const, fileName: `cms-exception-export-20260803-${String(sequence).padStart(2, '0')}.xlsx`, rowCount: rows.length, requestedBy: actorId, requestedAt: '2026-08-03T22:30:00.000Z' };
  ...
  async completeExport(requestId: string, roleCode: AdminRoleCode) {
    assertPermission(roleCode, 'exceptions.export');
    const request = mockData.exportRequests.find(({ id }) => id === requestId && id.startsWith('exception-export-'));
