...
export function useAdminTransactions(query: AdminTransactionQuery, roleCode: AdminRoleCode) {
  return useQuery({ queryKey: [...adminTransactionKeys.all, query, roleCode], queryFn: () => adminTransactionService.list(query, roleCode), retry: false });
}
...
export function useExportAdminTransactions() {
  return useMutation({
    mutationFn: async ({ query, roleCode, actorId }: { query: AdminTransactionQuery; roleCode: AdminRoleCode; actorId: string }) => {
      const request = await adminTransactionService.requestExport(query, roleCode, actorId);
