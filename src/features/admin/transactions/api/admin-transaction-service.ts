...
function assertView(roleCode: AdminRoleCode) {
  if (!hasPermission(roleCode, 'transactions.view'))
    throw new Error('FORBIDDEN');
}

function filteredTransactions(query: AdminTransactionQuery) {
  if (Object.keys(validateTransactionDateRange(query)).length)
  ...
  return mockData.transactions.filter((transaction) => {
    ...
    if (query.status !== 'ALL' && transaction.status !== query.status)
      return false;
    if (
      query.dateFrom &&
...
function project(
  transaction: (typeof mockData.transactions)[number],
  roleCode: AdminRoleCode,
) {
  const tenant = mockData.tenants.find(
    ({ id }) => id === transaction.tenantId,
  )!;
  ...
  const items = mockData.transactionItems.filter(
    ({ transactionId }) => transactionId === transaction.id,
  );
  const estimatedGrossCommission = items.reduce(
    (sum, item) =>
      sum + (item.status === 'REFUNDED' ? 0 : item.grossCommission),
    0,
  );
  ...
  const actualGrossCommission = items.reduce(
    (sum, item) =>
      sum + (item.status === 'CONFIRMED' ? item.grossCommission : 0),
    0,
  );
  ...
  return {
    ...
    estimatedGrossCommission:
      showGross && transaction.status === 'PENDING'
        ? estimatedGrossCommission
        : undefined,
    estimatedTenantShare:
      showTenantShare && transaction.status === 'PENDING'
        ? estimatedTenantShare
        : undefined,
    actualGrossCommission:
      showGross && transaction.status !== 'CANCELLED' && actualGrossCommission
        ? actualGrossCommission
        : undefined,
    actualTenantShare:
      showTenantShare && transaction.status !== 'CANCELLED' && actualTenantShare
        ? actualTenantShare
        : undefined,
    affiliateKeep: showAffiliateKeep
      ? items.reduce((sum, item) => sum + item.affiliateKeep, 0)
      : undefined,
  };
}
...
export const adminTransactionService = {
  async list(query: AdminTransactionQuery, roleCode: AdminRoleCode) {
    ...
    items.sort((left, right) => {
      const comparison = String(left[query.sortBy]).localeCompare(
    ...
    return structuredClone({
      items: items
        ...
        .map((item) => project(item, roleCode)),
      page,
  ...
  async getDetail(transactionId: string, roleCode: AdminRoleCode) {
    ...
    const showTenantShare = canViewFinancialField(roleCode, 'tenantShare');
    const showAffiliateKeep = canViewFinancialField(roleCode, 'affiliateKeep');
    const items = mockData.transactionItems.flatMap((item) =>
      item.transactionId === transactionId
        ? [
            {
              ...item,
              brandCommissionValue: showGross
                ? item.brandCommissionValue
                : undefined,
              grossCommission: showGross ? item.grossCommission : undefined,
              tenantShareValue: showTenantShare
                ? item.tenantShareValue
                : undefined,
    ...
    return structuredClone({
      ...
      histories,
      financialScope: {
        grossCommission: showGross,
        tenantShare: showTenantShare,
        affiliateKeep: showAffiliateKeep,
      },
    });
  },
  ...
  async requestExport(
    ...
    if (!hasPermission(roleCode, 'transactions.export'))
      throw new Error('FORBIDDEN');
    const rows = filteredTransactions(query).map((item) =>
      project(item, roleCode),
    );
  ...
  async completeExport(requestId: string, roleCode: AdminRoleCode) {
    ...
    const request = mockData.exportRequests.find(
      ({ id }) =>
        id === requestId && requestId.startsWith('transaction-export-'),
    );
