...
import { DEMO_RESET_OTP, tenantResetEmailSchema, tenantResetOtpSchema, tenantResetPasswordSchema, type TenantResetPasswordInput } from '../model/tenant-password-recovery';
...
function getRequest(requestId: string) {
  const request = mockData.passwordResetRequests.find(({ id }) => id === requestId);
...
export const tenantPasswordRecoveryService = {
  async requestReset(input: { email: string }) {
    ...
    const account = mockData.authAccounts.find((item) => item.portalType === 'TENANT' && item.email.toLowerCase() === email.toLowerCase());
    if (!account || !account.tenantId) throw new Error('EMAIL_NOT_FOUND');
    const request = { id: `tenant-reset-${mockData.passwordResetRequests.length + 1}`, accountId: account.id, tenantId: account.tenantId, email: account.email, otpHash, issuedAt, expiresAt, attempts: 0, verifiedAt: null, completedAt: null };
    mockData.passwordResetRequests.push(request);
    mockData.auditRecords.push({ id: `audit-reset-${mockData.auditRecords.length + 1}`, actorId: account.id, action: 'REQUEST_PASSWORD_RESET', entityType: 'TENANT_ACCOUNT', entityId: account.id, occurredAt: issuedAt });
    return structuredClone({ id: request.id, email: request.email, expiresAt: request.expiresAt, demoOtp: DEMO_RESET_OTP });
  },
  ...
  async verifyOtp(input: { requestId: string; otp: string }) {
    ...
    return structuredClone({ requestId: request.id, verifiedAt: request.verifiedAt });
  },
  ...
  async resetPassword(input: TenantResetPasswordInput) {
    ...
    const account = mockData.authAccounts.find(({ id }) => id === request.accountId);
    ...
    request.otpHash = '';
    mockData.auditRecords.push({ id: `audit-reset-${mockData.auditRecords.length + 1}`, actorId: account.id, action: 'RESET_TENANT_PASSWORD', entityType: 'TENANT_ACCOUNT', entityId: account.id, occurredAt: request.completedAt });
