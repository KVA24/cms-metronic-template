...
## 6. Final regression and cleanup record

| Check                       | Result | Evidence summary                                                                                                                                                                   |
| --------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Legacy dependency inventory | Pass   | Removed the obsolete `account`, `activity-log`, `auth`, `config`, `dashboards` and `welcome` feature files plus their dedicated UI store. `rg` finds no imports or routes to them. |
| Root and removed routes     | Pass   | `/` resolves the first permitted page from the active portal session; removed `/dashboards` returns 404 and Return Home resolves correctly.                                        |
| `CMS_ADMIN`                 | Pass   | Full ADMIN navigation and Reporting Dashboard render after cleanup.                                                                                                                |
| `CMS_FINANCE`               | Pass   | Permitted dashboard and financial projection render after cleanup.                                                                                                                 |
| `CMS_OPERATION`             | Pass   | Dashboard retains gross commission and excludes Affiliate keep.                                                                                                                    |
| `CMS_CSKH`                  | Pass   | Direct Category access returns 403; Return Home resolves to the only permitted Transactions route and menu.                                                                        |
| Locale and responsive       | Pass   | ADMIN Transactions renders in Vietnamese at 390 × 844 with functional filters and horizontally accessible table.                                                                   |
| Browser console             | Pass   | No error, warning or browser issue on the final responsive role check.                                                                                                             |
| Automated gate              | Pass   | 248 tests, lint and production build pass after legacy deletion.                                                                                                                   |

The implementation evidence and A27 regression are complete. Human sign-off remains intentionally unchecked in the parent technical specification.
