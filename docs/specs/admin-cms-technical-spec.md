...
- [ ] All four ADMIN and four TENANT seed personas authenticate only in their portal.
- [ ] EN/VI switch and `/admin/*`/`/tenant/*` guards work without persistence.
- [ ] `npm run lint && npm test && npm run build` passes.
- [ ] Review shared contracts before feature services depend on them.
...
- [ ] Dashboard financial fields differ correctly by role.
